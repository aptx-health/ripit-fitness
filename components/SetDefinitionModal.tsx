...
export default function SetDefinitionModal({
  ...
  return (
    <div className="fixed inset-0 backdrop-blur-md bg-black/40 dark:bg-black/60 flex items-center justify-center z-50 p-4">
      <SetDefinitionForm onClose={onClose} {...formProps} />
    </div>
  )
}

type SetDefinitionFormProps = Omit<SetDefinitionModalProps, 'isOpen'>

function deriveIntensityType(initialSets?: SetDefinitionFormProps['initialSets']): 'RIR' | 'RPE' | 'NONE' {
  if (!initialSets || initialSets.length === 0) return 'NONE'
  const firstSet = initialSets[0]
  if (firstSet.rpe !== null) return 'RPE'
  if (firstSet.rir !== null) return 'RIR'
  return 'NONE'
}

function deriveInitialSets(
  initialSets?: SetDefinitionFormProps['initialSets'],
  intensityType?: 'RIR' | 'RPE' | 'NONE'
): PrescribedSetInput[] {
  ...
  const type = intensityType ?? deriveIntensityType(initialSets)
  return initialSets.map(set => ({
    setNumber: set.setNumber,
    reps: set.reps,
    intensityType: type,
    intensityValue: type === 'RPE' ? (set.rpe || undefined) :
                   type === 'RIR' ? (set.rir || undefined) :
                   undefined
  }))
}

function SetDefinitionForm({
  onClose,
  exerciseName,
  onSubmit,
  initialSets,
  ...
  const [sets, setSets] = useState(() => deriveInitialSets(initialSets, initialIntensity))
